...
public abstract class GenericPacket implements IPacket {
	 ...
	 *                         /
	 *                         public static DatagramSocket sendSocket() throws
	 *                         SocketException
	 *                         {
	...
	public void send(DatagramSocket sock) throws IOException {
		 ...
		 * DatagramPacket p = new DatagramPacket(pkt, pkt.length, address,
		 * mqtt_udp_defs.MQTT_PORT);
		 * Engine.throttle();
		 * sock.send(p);
	 ...
	 *                               /
	 *                               public static IPacket recv() throws
	...
	public static IPacket recv(DatagramSocket s) throws IOException, MqttProtocolException {
		...
		byte[] buf = new byte[mqtt_udp_defs.PKT_BUF_SIZE];
	...
	public String toString() {
		return String.format("MQTT/UDP packet of unknown type from '%s', please redefine toString in %s", from,
				getClass().getName());
	}
